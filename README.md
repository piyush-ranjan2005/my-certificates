# 🎓 My Certificates & Achievements

Welcome to my certificate archive.  
This repository showcases my **verified certifications, hackathon achievements, and academic accomplishments**, reflecting my continuous learning, professional growth, and active participation in innovation-driven programs.

---

## 📜 Certifications

| Certificate | Issuing Organization | Date |
|------------|----------------------|------|
| **Artificial Intelligence** | Infosys Springboard | June 2025 |
| **GenAI Powered Data Analytics Job Simulation** | Tata Group | Aug 2025 |
| **Oracle Certified AI Foundation Associate** | Oracle | Sep 2025 |
| **Introduction to Deep Learning** | Infosys Springboard | June 2025 |
| **Infosys Springboard Internship Project Completion Certificate- AI Domain** | Infosys Springboard | Feb 2025 |


📁 Individual certificates are available inside the `certificates/Professional-Certifications/` folder.

---

## 🏆 Hackathon Achievements

| Event | Achievement | Organizer | Date |
|------|------------|----------|------|
| **Internal Hackathon 5.0** | 🥉 3rd Place | COER University, Roorkee | Apr 2025 |
| **Hackverse** | 🏅 Consolation Prize(5th place) | IILM University, Gurugram | Apr 2025 |
| **Internal Hackathon 3.0** | 🎯 Participant | COER University, Roorkee | Nov 2024 |
| **Adobe India Hackathon** | 🎯 Participant | Adobe | 2025 |
| **Ideathon 1.0** | 🎯 Participant | Innovation and Incubation Cell COER University, Roorkee | 2025 |

📁 Supporting certificates can be found in the `certificates/Hackathons/` directory.

---

## 📂 Repository Structure

certificates/
├── Hackathons/
├── Professional-Certifications/


Each folder is organized for **easy navigation and verification**.

---

## 📬 Contact

If you’d like to connect or know more about my work:

- 📧 **Email:** [arpiyush2005@gmail.com](mailto:arpiyush2005@gmail.com)
- 🔗 **LinkedIn:** [LinkedIn Profile](https://www.linkedin.com/in/piyush-ranjan2005)
- 💻 **GitHub:** [GitHub Profile](https://github.com/piyush-ranjan2005)

---

✨ *This repository is continuously updated as I earn more certifications and achievements.*
